...
void __attribute__ ((weak)) dhd_plat_pm_runtime_init(void *plat_info,
		struct pci_dev *pdev)
{
	return;
}

void __attribute__ ((weak)) dhd_plat_pm_runtime_exit(void *plat_info)
{
	return;
}
...
int __attribute__ ((weak)) dhd_plat_get_wlan_host_wake_up_gpio(void)
{
	return BCME_ERROR;
}
